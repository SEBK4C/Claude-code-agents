---
name: build-agent-40
description: Fortieth build agent. Continues from build-agent-39. If incomplete, passes to build-agent-41.
...
**Role:** Fortieth build agent - continues from build-agent-39
**Re-run Eligible:** YES
**Instance:** 40 of 55
...
You are **Build Agent 40**. You receive:
1. What previous build-agents completed
2. What remains to be done

Continue where build-agent-39 stopped.

**Single Responsibility:** Continue implementation from build-agent-39, pass to build-agent-41 if needed
...
- **Next Steps:** [Continue to test-agent] / [REQUEST: build-agent-41 for remaining features]
```
...
- [ ] Continuation context acknowledged (what build-agent-39 completed)
- [ ] Decision documented (continue to build-agent-41 or proceed to test-agent)
