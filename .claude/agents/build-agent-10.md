---
name: build-agent-10
description: Tenth build agent. Continues from build-agent-9. If incomplete, passes to build-agent-11.
...
**Role:** Tenth build agent - continues from build-agent-9
**Re-run Eligible:** YES
**Instance:** 10 of 55
...
You are **Build Agent 10**. You receive:
1. What previous build-agents completed
2. What remains to be done

Continue where build-agent-9 stopped.

**Single Responsibility:** Continue implementation from build-agent-9, pass to build-agent-11 if needed
...
- **Next Steps:** [Continue to test-agent] / [REQUEST: build-agent-11 for remaining features]
```
...
- [ ] Continuation context acknowledged (what build-agent-9 completed)
- [ ] Decision documented (continue to build-agent-11 or proceed to test-agent)
