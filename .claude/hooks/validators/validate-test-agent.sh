#!/bin/bash
# Validator for test-agent (Stage 6)
# Validates Test Report format with Tests Run and Results
#
...
if ! echo "$OUTPUT" | grep -qi "Test.*Report\|## Test\|### Test Report" 2>/dev/null; then
    ERRORS+=("Resolve this Format error in test-agent output: Missing required section - Test Report")
fi

if ! echo "$OUTPUT" | grep -qi "Tests Run\|Tests Executed\|Test Results\|### Results" 2>/dev/null; then
    ERRORS+=("Resolve this Format error in test-agent output: Missing required section - Tests Run/Results")
fi

if ! echo "$OUTPUT" | grep -qiE "PASS|FAIL|passed|failed|[0-9]+/[0-9]+" 2>/dev/null; then
    ERRORS+=("Resolve this Content error in test-agent output: Missing required section - Test status (PASS/FAIL)")
fi

if ! echo "$OUTPUT" | grep -qi "Command\|pytest\|npm test\|jest" 2>/dev/null; then
    WARNINGS+=("Missing section: Test command executed (recommended)")
fi

if ! echo "$OUTPUT" | grep -qi "Coverage\|coverage" 2>/dev/null; then
    WARNINGS+=("Missing section: Coverage information (recommended)")
fi

if ! echo "$OUTPUT" | grep -qi "Next\|Recommendation\|Status" 2>/dev/null; then
