#!/bin/bash
# Validator for plan-agent (Stage 2)
# Validates Implementation Plan format with Batches and File Mappings
#
...
if ! echo "$OUTPUT" | grep -qi "Implementation Plan\|## Plan\|### Plan" 2>/dev/null; then
    ERRORS+=("Resolve this Format error in plan-agent output: Missing required section - Implementation Plan")
fi

if ! echo "$OUTPUT" | grep -qi "Batch\|### Batch\|## Batch" 2>/dev/null; then
    ERRORS+=("Resolve this Format error in plan-agent output: Missing required section - Batches")
fi

if ! echo "$OUTPUT" | grep -qiE "Files to|File Mappings|Files Changed|Create:|Modify:" 2>/dev/null; then
    ERRORS+=("Resolve this Format error in plan-agent output: Missing required section - File Mappings")
fi

if ! echo "$OUTPUT" | grep -qiE "F[0-9]+|Feature" 2>/dev/null; then
